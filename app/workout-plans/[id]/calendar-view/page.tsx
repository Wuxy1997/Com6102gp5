...
import { AlertCircle, Calendar, ArrowLeft, Clock, Info, X, CheckCircle } from "lucide-react"
...
export default function WorkoutPlanCalendarView({ params }: { params: { id: string } }) {
  ...
  // Fetch completions for this workout plan
  ...
  // Check for auto-completion of exercises
  useEffect(() => {
    if (!user) return

    // Function to check for auto-completions
    const checkAutoCompletions = async () => {
      try {
        const response = await fetch("/api/workout-plans/auto-complete", {
          method: "POST",
          headers: {
        ...
        if (response.ok) {
          ...
          if (data.completedWorkouts && data.completedWorkouts.length > 0) {
            ...
            // Notify user of auto-completions
            data.completedWorkouts.forEach((workout: any) => {
              toast({
                title: "Workout Auto-Completed",
                description: `${workout.exerciseName} (${workout.duration} minutes, ${workout.caloriesBurned} calories)`,
                variant: "default",
              })
            })
          }
        ...
        console.error("Error checking auto-completions:", error)
      }
    }
    ...
    const interval = setInterval(checkAutoCompletions, 5 * 60 * 1000)
  ...
  useEffect(() => {
    ...
    // Convert to our format (0 = Sunday, 6 = Sunday)
  ...
  const getExercisesStartingAt = (day: string, timeSlot: string): CalendarExercise[] => {
    ...
    return dayExercises
      ...
      .map((exercise) => {
        ...
        const isCompleted = completions.some((completion) => completion.exerciseName === exercise.name)
      ...
      <main className="container mx-auto py-6 px-4 pt-20">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
          <div>
            ...
            <p className="text-muted-foreground">View your weekly workout schedule</p>
          </div>
        ...
        <div className="grid grid-cols-1 gap-6">
          <Card className="border-0 bg-gray-950 text-white shadow-md">
            <CardHeader>
              ...
              <CardDescription className="text-gray-400">
                Workouts are automatically marked as complete when their scheduled time has passed
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full border-collapse">
                  <thead>
                    <tr>
                      <th className="border border-gray-800 p-2 bg-gray-900 sticky left-0 z-10">Time</th>
                      {DAYS_OF_WEEK.map((day) => (
                        <th key={day} className="border border-gray-800 p-2 bg-gray-900">
                          {day}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {TIME_SLOTS.map((timeSlot) => (
                      <tr key={timeSlot.id}>
                        <td className="border border-gray-800 p-2 bg-gray-900 font-medium text-sm sticky left-0 z-10">
                          {timeSlot.label}
                        </td>
                        {DAYS_OF_WEEK.map((day) => {
                                ...
                                {exercisesStartingHere.map((exercise, index) => {
                                  ...
                                  return (
                                    <div
                                      ...
                                      } transition-colors p-1 overflow-hidden text-xs`}
                                      style={{
                                        top: 0,
                                        height: `${height * 100}%`,
                                        zIndex: 1,
                          ...
                          // If this slot is covered by an exercise but not the start, render empty
                            ...
                            const isCompleted = exercisesCoveringHere.some((ex) => ex.isCompleted)
                            return (
                              <td
                                key={`${day}-${timeSlot.id}`}
                                className={`border border-gray-800 p-0 ${
                                  isCompleted ? "bg-green-900/30" : "bg-blue-900/30"
                                }`}
                                onClick={() => openExercisesDialog(day, timeSlot.id)}
                              />
                            )
        ...
        <DialogContent className="max-w-md bg-gray-900 text-white border-gray-800">
          <DialogHeader>
            ...
            <DialogDescription className="text-gray-400">
              Exercises scheduled for this time slot are automatically completed when their time passes
            </DialogDescription>
          </DialogHeader>
              ...
              {selectedExercises.map((exercise, index) => (
                <div
                  ...
                  className={`p-3 border rounded-md ${
                    exercise.isCompleted ? "border-green-800 bg-green-900/20" : "border-gray-800 bg-gray-800/50"
                  }`}
                >
                  <h3 className="font-medium flex items-center">
                    {exercise.isCompleted && <CheckCircle className="h-4 w-4 mr-2 text-green-400" />}
                    {exercise.name}
                    {exercise.isCompleted && (
