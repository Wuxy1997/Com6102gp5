  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        router.push(`/workout-plans/${params.id}`)
      } else {
        const errorData = await response.json()
        setError(errorData.error || "Failed to update workout plan")
      }
    } catch (error) {
      console.error("Error updating workout plan:", error)
      setError("An error occurred while updating the workout plan")
    } finally {
      ...
      <main className="container mx-auto py-6 px-4 pt-20">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
          <div>
            ...
            <p className="text-muted-foreground">Update your workout program</p>
          </div>
